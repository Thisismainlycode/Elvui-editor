...
$('#import').onclick=()=>{
 ...
 $('#read-profile').onclick=()=>{try{const result=parseInput($('#import-text').value);$('#import-error').textContent='';$('#profile-choice').innerHTML=`<label class="field">Choose a profile<select id="choose-profile">${result.profiles.map((p,i)=>`<option value="${i}">${esc(p.name)}</option>`).join('')}</select></label><p class="muted">Loading replaces the working profile. You can undo it.</p><button id="load-profile" class="primary">Load selected profile</button>`;$('#load-profile').onclick=()=>{const p=result.profiles[Number($('#choose-profile').value)];mutate(()=>{profile=clone(p.profile);name=p.name;source=`Imported ${p.format}`;selected='player';},`Loaded ${p.name}; original file unchanged`);$('#profile-name').value=name;$('#dialog').close();};}catch(e){$('#profile-choice').innerHTML='';$('#import-error').textContent=e.message;}};
};
