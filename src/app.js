...
function render(){
 ...
 const active=visible.filter(f=>f.enabled),off=visible.length-active.length;$('#count').textContent=`${active.length} visible${off?` · ${off} off`:''}${custom.length&&!showCustom?` · ${custom.length} custom`:''}`;$('#profile-status').textContent=source+(dirty?' · Edited':'');$('#undo').disabled=!history.length;$('#redo').disabled=!future.length;
