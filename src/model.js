...
const bar=(n,x=0,y=-370)=>({id:`bar${n}`,label:`Action bar ${n}`,mover:`ElvAB_${n}`,kind:'bar',path:['actionbar',`bar${n}`],x,y,w:406,h:32,enable:true,enableKey:'enabled',defaultEnabled:DEFAULT_ENABLED_BARS.has(n),defaultButtons:n===3||n===5?6:12,defaultCols:n===4?1:n===3||n===5?6:12});
...
export function dimensions(p,d){
 ...
 if(d.kind==='bar'){const n=get(p,[...path,'buttons'],d.defaultButtons),cols=get(p,[...path,'buttonsPerRow'],d.defaultCols),size=get(p,[...path,'buttonSize'],32),height=get(p,[...path,'keepSizeRatio'],true)===false?get(p,[...path,'buttonHeight'],32):size,gap=get(p,[...path,'buttonSpacing'],2);w=Math.min(n,cols)*size+(Math.min(n,cols)-1)*gap;h=Math.ceil(n/cols)*height+(Math.ceil(n/cols)-1)*gap;}
