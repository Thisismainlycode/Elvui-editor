...
function benikUIDefinitions(p){
 ...
 const add=d=>{if(get(movers,[d.mover])!==undefined)defs.push({...d,plugin:'BenikUI'});};
 ...
 for(const [key,mover,label,width] of BENIKUI_DASHBOARDS){const path=['benikui','dashboards',key];if(key==='system'){const orientation=get(p,[...path,'orientation'],'BOTTOM'),rows=get(p,[...path,'chooseSystem'])instanceof Map?[...get(p,[...path,'chooseSystem']).values()].filter(Boolean).length:5,w=get(p,[...path,'width'],width),h=benikUIDashboardHeight(p,key);add({id:`benikui-dashboard-${key}`,label,mover,kind:'benikui',subkind:'dashboard',path,enable:true,defaultEnabled:true,w:orientation==='BOTTOM'?w:w*Math.max(1,rows)+(Math.max(1,rows)-1)*get(p,[...path,'spacing'],1),h,x:-750,y:360});}else add({id:`benikui-dashboard-${key}`,label,mover,kind:'anchor',subkind:'dynamic dashboard',path,enable:true,defaultEnabled:true,w:12,h:12,x:0,y:0,anchorOnly:true});}
