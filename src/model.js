...
export function actionBarLayout(p,d){const path=d.path,n=get(p,[...path,'buttons'],d.defaultButtons),cols=Math.max(1,Math.min(n,get(p,[...path,'buttonsPerRow'],d.defaultCols))),size=get(p,[...path,'buttonSize'],32),height=get(p,[...path,'keepSizeRatio'],true)===false?get(p,[...path,'buttonHeight'],32):size,gap=get(p,[...path,'buttonSpacing'],2),rows=Math.ceil(n/cols);return {n,cols,size,height,gap,rows,w:cols*size+(cols-1)*gap,h:rows*height+(rows-1)*gap};}
