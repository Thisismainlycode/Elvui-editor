...
export function newProfile(){const p=new Map();for(const d of DEFINITIONS){set(p,['movers',d.mover],`CENTER,UIParent,CENTER,${d.x},${d.y}`);if(d.enable)set(p,[...d.path,d.enableKey||'enable'],true);if(d.resize){if(d.kind==='minimap')set(p,[...d.path,'size'],d.w);else{set(p,[...d.path,'width'],d.w);set(p,[...d.path,'height'],d.h);}}if(d.kind==='bar'){set(p,[...d.path,'buttons'],12);set(p,[...d.path,'buttonsPerRow'],12);set(p,[...d.path,'buttonSize'],34);set(p,[...d.path,'buttonSpacing'],2);}}set(p,['chat','panelWidth'],400);set(p,['chat','panelHeight'],180);return p;}
...
export function dimensions(p,d){
 ...
 if(d.kind==='bar'){const n=get(p,[...path,'buttons'],12),cols=get(p,[...path,'buttonsPerRow'],12),size=get(p,[...path,'buttonSize'],34),height=get(p,[...path,'keepSizeRatio'],true)===false?get(p,[...path,'buttonHeight'],size):size,gap=get(p,[...path,'buttonSpacing'],2);w=Math.min(n,cols)*size+(Math.min(n,cols)-1)*gap;h=Math.ceil(n/cols)*height+(Math.ceil(n/cols)-1)*gap;}
