...
export const DEFINITIONS=[unit('player','Player','ElvUF_PlayerMover',-320,-180),unit('target','Target','ElvUF_TargetMover',320,-180),unit('focus','Focus','ElvUF_FocusMover',-320,-95,180,36),unit('pet','Pet','ElvUF_PetMover',-320,-250,180,30),{id:'bar1',label:'Action bar 1',mover:'ElvAB_1',kind:'bar',path:['actionbar','bar1'],x:0,y:-370,w:430,h:34,enable:true},{id:'bar2',label:'Action bar 2',mover:'ElvAB_2',kind:'bar',path:['actionbar','bar2'],x:0,y:-326,w:430,h:34,enable:true},{id:'minimap',label:'Minimap',mover:'MinimapMover',kind:'minimap',path:['general','minimap'],x:805,y:380,w:170,h:170,resize:true},{id:'leftchat',label:'Left chat',mover:'LeftChatMover',kind:'chat',x:-725,y:-355,w:400,h:180},{id:'rightchat',label:'Right chat',mover:'RightChatMover',kind:'chat',x:725,y:-355,w:400,h:180},{id:'raid1',label:'Raid group',mover:'ElvUF_Raid1Mover',kind:'raid',path:['unitframe','units','raid1'],x:-745,y:0,w:300,h:190}];
export function newProfile(){const p=new Map();for(const d of DEFINITIONS){set(p,['movers',d.mover],`CENTER,UIParent,CENTER,${d.x},${d.y}`);if(d.enable)set(p,[...d.path,'enable'],true);if(d.resize){if(d.kind==='minimap')set(p,[...d.path,'size'],d.w);else{set(p,[...d.path,'width'],d.w);set(p,[...d.path,'height'],d.h);}}if(d.kind==='bar'){set(p,[...d.path,'buttons'],12);set(p,[...d.path,'buttonsPerRow'],12);set(p,[...d.path,'buttonSize'],34);set(p,[...d.path,'buttonSpacing'],2);}}set(p,['chat','panelWidth'],400);set(p,['chat','panelHeight'],180);return p;}
...
export function dimensions(p,d){
 ...
 if(d.kind==='bar'){const n=get(p,[...path,'buttons'],12),cols=get(p,[...path,'buttonsPerRow'],12),size=get(p,[...path,'buttonSize'],34),gap=get(p,[...path,'buttonSpacing'],2);w=Math.min(n,cols)*size+(Math.min(n,cols)-1)*gap;h=Math.ceil(n/cols)*size+(Math.ceil(n/cols)-1)*gap;}
...
export function framesFor(p,viewport){
 const defs=[...DEFINITIONS];const movers=get(p,['movers']);if(movers instanceof Map)for(const [m]of movers)if(typeof m==='string'&&!defs.some(d=>d.mover===m))defs.push({id:m,label:m,kind:'unknown',mover:m,w:160,h:40,x:0,y:0});
 const frames=defs.map(d=>{const raw=get(p,['movers',d.mover]);return {...d,...dimensions(p,d),enabled:d.enable?get(p,[...d.path,'enable'],true)!==false:true,moverData:parseMover(raw),raw,estimated:raw===undefined};});
 ...
 function resolve(f,seen=new Set()){
  ...
  const a=anchorFraction(m.anchor),b=anchorFraction(m.relative);f.rect={left:parent.left+parent.w*b[0]+m.x-f.w*a[0],top:parent.top+parent.h*b[1]-m.y-f.h*a[1],w:f.w,h:f.h};return f.rect;
 }
